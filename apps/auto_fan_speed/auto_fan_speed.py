   ...
   # USER PREFERENCES
    if "speeds" in self.args:
     ...
     def configure(self, kwargs):
       ...
       init_log += [f"SPEEDS        OFF < {self.low} > LOW < {self.medium} > MEDIUM < {self.halfway} > HALFWAY < {self.high} > HIGH\n"]
    ...
    # if sun is above horizon, then add offset
           ...
           self.debug_log(f"AUTO FAN SPEED: ({str(room_temperature)}/({str(fan_speed_percentage)}%") + (" (SUN OFFSET)" if sun_above_horizon >
           ...
           def hvac_daily_shut_off(self, kwargs):
            ...
            def is_time_okay(self, start, end):
             ...
             if (start < end):
              return start <= current_time and current_time <= end or start <= current_time or current_time <= end
